...
@section('main-content')
    <div id="detail-container" style="margin-top: 30px">
        ...
        <div class="detail-vid-des">
            <div class="detail-vid-judul">{{ $video->judul }}</div>
        ...
        <div class="detail-vid-video">
            <iframe width="900" height="400" src="{{ $video->link_embed }}" frameborder="0" allowfullscreen></iframe>
        </div>

        {{-- DETAIL DESKRIPSI --}}
        <div class="detail-vid-des">
            <div class="detail-isi">{{ $video->deskripsi }}</div>
        </div>
            ...
            <div class="detail-vid-terkait">
                ...
                <div class="videos-content">
                    @foreach($videos as $video)
                        <div style="height: 280px" class="videos" onclick="navigateTo('{{ route('video-detail', ['id' => $video->id]) }}')">
                            <div class="video-container" style="height: 50%">
                                <iframe width="560" height="100%" src="{{ $video->link }}" frameborder="0" allowfullscreen></iframe>
                            </div>
                            <div class="video-description" style="height: 50%; padding:20px">
                                <p class="video-type" style="margin-bottom: 1em"><b>{{ $video->jenis }}</b></p>
                                <p class="judul" style="margin-bottom: 1em">{{ strlen($video->judul) > 80 ? substr($video->judul, 0, 80) . '...' : $video->judul }}</p>
                                <div class="video-info" style="display: flex; align-items: center">
                                    <i class="fa fa-clock-o" style="color: #2a8939; margin-right: 0.5em"></i>
                                    <p>{{ \Carbon\Carbon::parse($video->tanggal_upload)->translatedFormat('d F Y') }}</p>
                                </div>
