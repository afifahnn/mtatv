...
@section('main-content')
    <div id="detail-container" style="margin-top: 30px">
        ...
        <div class="detail-info" style="padding: 52px">
            <button style="margin-right: 2em; font-family: Poppins" onclick="goBack()">
            ...
            <div class="detail-links">
                ...
                <a>{{ ucwords($jihad->judul) }}</a>
            </div>
        </div>

        {{-- DETAIL JUDUL --}}
        <div class="detail-vid-des">
            <div class="detail-vid-judul">{{ $jihad->judul }}</div>
            <div class="video-info" style="display: flex; align-items: center">
                <i class="fa fa-clock-o" style="color: #2a8939; margin-right: 0.5em"></i>
                <p>{{ \Carbon\Carbon::parse($jihad->tanggal_upload)->translatedFormat('d F Y') }}</p>
            </div>
        </div>

        {{-- DETAIL VIDEO --}}
        <div class="detail-vid-video">
            <iframe width="900" height="400" src="{{ $jihad->link }}" frameborder="0" allowfullscreen></iframe>
        </div>

        {{-- DETAIL DESKRIPSI --}}
        <div class="detail-vid-des">
            <div class="detail-isi">{{ $jihad->deskripsi }}</div>
        </div>
        ...
        <div class="detail-vid-terkait">
            ...
            <div class="videos-content">
                @foreach($jihads as $videos)
                    <div style="height: 280px" class="videos" onclick="navigateTo('{{ route('jihad-pagi-detail', ['id' => $videos->id]) }}')">
                        <div class="video-container" style="height: 50%">
                            <iframe width="300" height="100%" src="{{ $videos->link }}" frameborder="0" allowfullscreen></iframe>
                        </div>
                        <div class="video-description" style="height: 50%; padding:20px">
                            <p class="jenis" style="margin-bottom: 1em"><b>Jihad Pagi</b></p>
                            <p class="judul" style="margin-bottom: 0.5em">{{ strlen($videos->judul) > 80 ? substr($videos->judul, 0, 80) . '...' : $videos->judul }}</p>
